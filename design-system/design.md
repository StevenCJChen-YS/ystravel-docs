...
- **整頁沒有工具列的頁，不要為了一顆鈕硬掛 `DataToolbar`**：它的搜尋框是**無條件渲染**的
  （只有篩選／動作那一列有 `v-if`），掛上去會憑空多一個沒有作用的搜尋框。
  代表案例＝權限覆寫頁，見下方旁註。

> **⚠️ 用了 `useResponsiveColumns` 的表格，就一定要配「欄位顯示」選單。**
> 那支 composable 的檔頭自己寫著「使用者仍可用『欄位顯示』下拉自己加回」——
> **它的設計前提就是選單存在**。少了選單，窄視窗下被自動藏起來的欄
> **再也叫不回來**，而畫面上完全正常、沒有任何東西會報錯。
> 2026-09-09 的顧客諮詢就是這樣漏的：1024–1279px 時「詢問內容」「最新進度」
> 自動隱藏，而那一頁沒有選單 ⇒ 使用者只能把視窗拉寬才看得到（#288 補上）。
>
> **例外（已拍板）：權限覆寫頁（`UserPermissionOverridesPage`）刻意不補**（Steven 2026-09-09）。
> 那一頁**整頁沒有工具列**——沒有搜尋、沒有篩選、沒有匯出，只有一張表。
> 為了一顆選單掛上 `DataToolbar` 會生出一個沒有作用的搜尋框（見上一條）；
> 做了兩版給他看畫面，結論是「感覺有點怪」。
>
> ⚠️ **代價寫在這裡，因為它是知情的取捨、不是漏掉**：那一頁 `<lg` 會藏
> 「誰設的」「什麼時候設的」、`<sm` 再藏「到期日」，沒有選單就叫不回來。
> 可接受的理由是那三欄都是輔助資訊——**判斷一筆覆寫是什麼，靠的是「權限」與「原因」兩欄**
> （頁面裡的註解就是這樣寫的）。
> 📌 哪天覺得窄視窗看不到那兩欄很煩，**正確的修法不是硬掛 `DataToolbar`**，
> 而是讓那顆選單能單獨使用（或給 `DataToolbar` 一個「不要搜尋框」的開關）。
>
> 🚧 **還沒拍板的一個：`OptionsManager`（選項設定）。** 它用了
> `useResponsiveColumns({ order: 'md' })` 卻沒有選單 ⇒ `<md` 時「排序」欄叫不回來。
> 但它**有工具列**（`#actions` 裡已經有「調整順序」），所以沒有上面那個「沒地方放」的理由，
> 補一顆是小事。2026-09-10 全站掃出來的，**還沒決定要不要補**。
>
> <sub>掃描方式：`useResponsiveColumns` 的呼叫點全在 `.vue`，逐檔比對有沒有 `ColumnVisibilityMenu`。
> 2026-09-10 的結果是 12 個有、2 個沒有（權限覆寫、OptionsManager）。</sub>
...
**欄標題就寫「操作」，不留白**（2026-09-09 補）。內容置中的話 `th` 跟著 `text-center`。

> ⚠️ **這條是被漏掉才發現的**：全站十個表格頁裡八頁寫「操作」、**兩頁（顧客諮詢、企業列表）
> 是空字串**，而同模組的客戶總表是有的——所以不是刻意留白，是漏了。
> Steven 2026-09-09 看畫面問「為啥操作欄位的操作兩字消失了」才撈出來。
> 📌 **留白不會有任何東西報錯**，`header: ''` 是完全合法的值；而且**沒有人會 grep 一個空字串**。
> 這與本節底下那個附註是同一件事的兩面：那條講「規則只寫在程式註解裡＝規則不存在」，
> 這條講**「多數頁面都這樣做」也不等於規則存在**。
> **唯一的例外：那一欄只有一顆鈕時，標題可以直接寫那顆鈕的動作**（稽核紀錄的「查看」）
> ——比「操作」有資訊。⚠️ 判準是**顆數**不是好不好看；多出第二顆就回到「操作」。
> 擋件＝`scripts/check-table-actions-header.mjs`（例外要進它的 `EXEMPT` 並寫理由）。
...
#### 5.5.1.1 欄位多的長清單在 `2xl` 可以開三欄（`wide`，2026-08-18 新增）

**opt-in，不是預設。** 只給欄位多到「兩欄還是拉出一條長卡片」的使用點
（平台目前＝CRM 客戶詳情、企業詳情）。

**斷點是 `2xl`(1536) 不是 `xl`(1280)**，因為值的可用寬（一格扣掉 padding 40 ＋ 標籤欄 80 ＋ gap 12）：
...
而一個真實的公司 Email（`steven_chen@ystravel.com.tw`）＝ **197px**
⇒ 1280 開三欄必折行、1536 才放得下（餘裕 54px）。
**成效**：客戶詳情 449px（10 列）→ 359px（8 列）。

⚠️ **欄位少的使用點不要開**：實測帳號中心任職資料（10 個單值欄）三欄後最後一列只剩一格、
右邊空兩格；hr 公司資訊卡的「電話…／傳真…」那格被擠成兩行。**三欄的好處只在欄位夠多時存在。**

⚠️ **量之前先確認資料代表得了真實情況**：dev 的客戶 Email 全是空的（顯示「—」），
不塞一筆真實長度進去，這件事在畫面上完全看不出來。

**`span: 2`（固定占兩欄）**：補三欄時的落單格。兩欄版 `col-span-2` 就是整列、行為不變，
三欄版則與左邊那個落單格拼成完整一列（客戶詳情的備註＋台胞證效期）。

##### 實作雷（三個，都不會報錯）

1. **欄數不能只算一次。** CSS 沒辦法告訴 JS 現在是哪個斷點 ⇒ 同一份 rows 算兩遍，
   兩欄版發 `sm:` 的 class、三欄版發 `2xl:` 的覆寫。
2. **覆寫要「兩個方向都發」。** Tailwind 沒有「不寫就取消」——兩欄要畫、三欄不要的那一格
   得明確發 `2xl:border-r-0`（反之亦然）。只發一半的症狀是「某一格在 1536 以上多一條線」，
   而那條線看起來像本來就有。
3. **右分隔線的條件是「右邊真的還有一格」，不是「我在第幾欄」。**
   那一列沒排滿時（三欄放 10 格，最後一列剩 1 格），照欄位算會讓落單那格畫出
   **一條懸在空白裡的線**。⚠️ 兩欄版一直都有這個 bug，只是既有頁面剛好都排得滿、沒撞到。
   📌 **這是 §6.8 的定義列版本**（2026-08-25 升成通則，因為 `ModuleRail` 又犯了一次）：
   分隔線的條件一律是「**兩邊都有東西**」，不是「我這邊有東西」。
...
### 5.6.1 modal 的寬度級距（2026-08-19 訂）

> 起因：Steven 從畫面回報「內容很少的 modal 感覺不用那麼寬」。查下去發現全站沒有標準
> ——**33 個 modal 使用點用了 5 種寬度**，而在此之前唯一的規則只有 §6.7 一句「欄位少的用 md」，
> 沒說什麼叫欄位少。最明顯的症狀：**29 個確認框呼叫點裡有 10 個各自傳 `width` 收窄，
> 那 10 次覆寫就是預設值選錯邊的證據**（預設當時是 448，改成 384 後那 10 行全部刪掉）。

**四級。寬度由「最寬的那一列內容需要多寬」決定，不是由欄位數量：**

| 級 | class | px | 用在 | 例 |
|---|---|---|---|---|
| 1 | `sm:max-w-sm` | 384 | 一句話的確認、只有一格輸入 | `ConfirmModal` 預設、刪除企業、調整親友關係、命名我的篩選 |
| 2 | `sm:max-w-md` | 448 | 單欄表單（含「要挑一筆」的清單）| 篩選面板、匯出、貼標籤、標記親友、匯入政府行事曆、任職歷程 |
| 3 | `sm:max-w-2xl` | 672 | 雙欄表單 | 客戶／企業／員工的新增編輯、稽核詳情 |
| 4 | 由內容決定 | — | 網格挑選、大表格 | 名片背景圖庫（`4xl`／`6xl`）、權限矩陣（`5xl`）|

**`sm:max-w-lg`（512）＝ Nuxt UI 的預設值，一律不要停在那裡。**
停在預設等於**沒有人決定過寬度**——2026-08-19 收斂前有三個 modal 是這樣，
量過之後沒有一個需要 512。

⚠️ **只有第 4 級要在使用點寫一行理由**；1～3 級照表用、不必寫。

**怎麼驗（動手前先做這一步）**：把 `max-width` 往下調一階，**看文字行數有沒有變**。
沒變＝那一階全是空白，可以收；變了才是內容真的需要那個寬度。
2026-08-19 實測：刪除企業在 320／384／448 三階框高**一律 253px**；調整親友關係 **288 就裝得下**，
384→512 之間一律 269px。**反例**：移除親友的確認在 448 剛好一行、384 會變兩行（201→221px）
——那一行是看過畫面後接受的代價，不是沒發現。

⚠️ **`<sm` 這些數字全部不生效**（modal 在手機是 `w-[calc(100vw-2rem)]`），
級距只影響 ≥640 的畫面。所以「手機上會不會太窄」不是選級距的理由。
...
**⚠️ 明文例外：對話框裡的主要動作用 `solid`，即使它是破壞性的**（2026-08-19 Steven 看畫面拍板）。
上表那一列管的是**頁面／列上的**破壞性鈕（它旁邊有一堆別的東西，soft 才不會搶）；
確認框裡只有兩顆鈕，取消是灰的、確認是紅的，**那個對比本身就是防誤觸的一部分**。

三個版本在同一個確認框裡並排實測（明暗都量）：

| | 亮色文字對比 | 亮色「鈕底 vs 白卡」 | 暗色文字對比 |
|---|---|---|---|
| `solid` | 4.53 ✅ | **4.53** | 4.53 ✅ |
| `soft` | 5.21 ✅ | **1.16** | 4.53 ✅ |
| `outline` | **3.75 ❌** | 1.00 | 5.20 ✅ |

- **`soft` 的底色其實看不見**：那層紅只有 10% 不透明度，對白卡 1.16、對暗卡 1.15
  ⇒ 它實際上是「紅字＋幾乎沒有底」，跟 `outline` 差別很小。
- **`outline` 的紅字在亮色只有 3.75，過不了 §2.7 的 AA 基線**（擋件 23）
  ⇒ **上表寫的 `outline` 只在暗色成立**，亮色要用，得先把 error 的文字色調深。

📌 這一條同時解釋了 `ConfirmModal` 從第一天就是 solid 紅——那不是漏改。
`FormModal` 的主要動作鈕曾經寫死 `primary`（於是刪除企業的鈕是綠的），2026-08-19 補上 `submitColor`。
...
| 服務**多行區塊**（標題＋說明），或**獨立存在** | `size-5`（20px） | 設定列（`AccountAppearancePage`）、`ModuleRail`、空狀態、載入 spinner |
| **整頁的狀態主角** | `size-6`（24px） | 回呼頁的成功／失敗／loading |
| 空白區的放置圖 | `size-7`+（28px+） | 頭像上傳的拖放區 |

**明文例外——提示類一律 `size-4`（16px）：**

| 元件 | 拍板 | 怎麼落地 |
|---|---|---|
| toast | Steven 2026-07-28 | `vite.config.ts` 的 toast theme |
| 提示框 `UAlert`（全站 38 個帶 icon 的使用點）| Steven 2026-09-24 | `vite.config.ts` 的 alert theme，一行全站生效 |
| 可關閉橫幅 `DismissibleBanner`（首頁 LINE 綁定）| Steven 2026-09-24 | 元件本身 |

它們結構上屬於「多行區塊」該用 20px，但**提示類的字都是 14px，20px 的記號對它們明顯偏重**。
toast 是三種尺寸並排實物比較後選的 16px（寬約 380px 的緊湊浮層，而設定列是頁面內的寬列——
**同一個 px 在兩者的感知重量不同**）；提示框是 Steven 看畫面發現同一個問題，要求全站提示類一起改。
這是刻意的例外，不是漏改；規則其餘部分（設定列、`ModuleRail`、空狀態…）不受影響。
...
>
> 提示框（2026-09-24）同一個道理：vertical 的 root 是 `items-start`，所以同樣補 `mt-0.5`，
> 但**放在 `variants.orientation.vertical` 而不是 `slots.icon`**——horizontal 是 `items-center`（整塊置中），
> 補了反而往下歪。可關閉橫幅是 `items-center` 的單行，**不用補**。
> 📌 **字級調到大／特大時，這幾顆會偏高 1.5／3px**：icon 尺寸走 `--spacing`、刻意不跟字級縮放（§3.4），
> 行高卻會長。改之前的 20px 對照組量出來**一模一樣**（2026-09-24 實測），所以是既有現象、不是縮小造成的；
> 要修得改成跟著行高算的上邊距，toast 與提示框一起改。
...
- **modal 寬度一律照 §5.6.1 的四級表**（2026-08-19 取代本條原本那句「欄位少的用 md」——
  那句沒定義什麼叫欄位少，於是全站長出五種寬度）。
...
### 6.8 分隔線：條件是「兩邊都有東西」（2026-08-25 定案）

> **分隔線分的是兩堆東西。只有一堆時它不是分隔線，是一條沒有理由的線。**

畫任何一條分隔線之前，條件要**同時**確認兩側：

```ts
// ✅ 兩邊都有東西才畫
const showDivider = computed(() => above.length > 0 && below.length > 0)

// ❌ 只確認了一邊——另一邊空的時候，線會懸在那裡
v-if="below.length"
```

**為什麼這條要寫下來**：它已經在**兩個形狀完全不同的地方**各犯一次，而且兩次都
①不會有任何東西報錯 ②只有「剛好權限／資料是那個組合」的使用者看得到：

| 犯在哪 | 錯的條件 | 症狀 |
|---|---|---|
| `ModuleRail` 的 rail 分隔線（2026-08-25） | 只判 `footerModules.length`（下半） | 沒有 hr／crm 權限的帳號**上半是空的**，線貼在 logo 底下什麼都沒分到 |
| `DefinitionList` 的欄間右分隔線（§5.5.1 實作雷 3） | 照「我在第幾欄」算 | 最後一列沒排滿時，落單那格畫出**一條懸在空白裡的線** |

**判準一句話：不要問「我這邊有沒有東西」，要問「我兩邊是不是各有東西」。**

📌 **常常空的那一側才是會出事的那一側**，而開發時你的帳號多半兩邊都有——
所以這種 bug 幾乎都是**別人的權限**才踩得到，自己怎麼看都正常。
驗法：把常空的那一側暫時清空跑一次（`.slice(0, 0)` 之類），看線有沒有跟著消失。
⚠️ 量的時候**不要用 `aria-hidden="true"` 去找分隔線**——每一顆 `UIcon` 都帶這個屬性，
會抓到 icon 而不是線（2026-08-25 實際量錯過一次）。認「帶 `border-t` 的直接子層」才對。
...
- **第二人稱一律「您」**（後端信件範本本來就是）。程式註解不受此限。
  **2026-08-24 起由 CI 擋件** `scripts/check-copy-pronoun.mjs`（掃前端 `.vue`／`.ts`，先挖掉註解再掃）。
  ⚠️ 補這支的理由是它**漏了一個多月才被發現**：規則 7/20 就寫了，而 7/29／8/06／8/18 三片
  各帶了「你」進來，`git log -S` 查出來**其中兩片是先寫進 PRD/SD、實作照抄**——
  ➡️ **寫規格時就要套這一節**，不要等實作。而且寫錯不會有任何東西報錯（typecheck／測試／build 全過）。
  📌 更好的做法是**整句不用人稱**：「決定每類通知要用哪些方式通知您」→「每類通知要用哪些方式送達」。
- 回應與文件輸出使用**繁體中文（台灣用語）**。

#### 10.2.1 句號（2026-08-24 Steven 拍板，全站一條規則）

> **單句不加句號；兩句以上每句都加**（含最後一句）。問號、驚嘆號比照句號算「一句」。

適用**所有**使用者看得到的字：頁面副標、ⓘ 說明、modal 副標、欄位說明、空狀態、提示條、toast、確認框。

- 好：`決定誰會收到哪一類通知`（單句，不加）
- 好：`這是目前的預設背景，不能停用。要下架它請先把預設指定給其他素材。`（兩句，都加）
- 壞：`修改名稱與分類，或切換啟用狀態。`（單句卻加了）
- 壞：`這是目前的預設背景，不能停用。要下架它請先把預設指定給其他素材`（多句卻漏最後一個，讀起來像漏打）

**為什麼是這一條**（2026-08-24 查了三家設計系統）：業界沒有人「全站都加」或「全站都不加」，
一律**按元素類型分**——[Microsoft](https://learn.microsoft.com/en-us/style-guide/punctuation/) 與
[GitLab Pajamas](https://design.gitlab.com/content/punctuation) 是「標題／標籤不加、說明文字是完整句就加」，
[Material](https://m1.material.io/style/writing.html) 則多一條「說明文字只有一句時也不加」。
本專案取 Material 那條，理由是它與 2026-08-19 已拍板的「頁面副標不加句號」相容，另一派要收回那個決定。

⚠️ **分號不算斷句**：`留空＝長期有效；臨時代理、專案期間才設期限` 是**一句**，不加。

📌 2026-08-24 一次收斂 63 處（全部是「單句卻帶句號」）。**確認框（`useConfirm`）那批 Steven 當天
決定先不動**，所以那裡目前仍有 14 句單句帶句號——恢復這件事時記得那是刻意留的，不是漏掃。
...
- **副標講「這頁管什麼」，不是「這個名詞是什麼」**（2026-08-19 新增）。
  ⚠️ 特別擋一種：**把 `CONTEXT.md` 的定義原封不動貼上去**。
  事故：企業總表寫「會帶一群人來的單位：公司、學校、機關、社團」——那是詞彙表裡
  用來跟 hr 的「公司」區分的定義句，貼到畫面上就變成**全站唯一一頁在解釋名詞**
  （客戶「個人旅客的基本資料與負責業務」、員工管理「員工主檔的建立、維護與入到職」都是講這頁管什麼），
  而且 21 全形字＋冒號串兩個子句，同時違反上面兩條。
  ➡️ **要讓人懂那個詞，列具體例子比寫定義快**：改成「公司、學校、機關的基本資料與聯絡人」。
  📌 判準：唸一遍副標，如果它在回答「這是什麼意思」而不是「這頁我能做什麼」，就寫錯了。
...
56. **動作鈕與資料 chip 放在同一個 `flex-wrap` 容器**（見 §5.2.3 ③）——要拆成上下兩塊
    ...
    **chip 只有 1～2 顆那顆鈕會真的跑進 chip 那一排**，多了才換行
    ...
    ⚠️ 這一條**桌機也成立**，不是手機專屬
57. **modal 沒有照 §5.6.1 的四級表選寬度**——尤其**停在 `sm:max-w-lg`（512，Nuxt UI 的預設值）**
    ＝沒有人決定過寬度。⚠️ 也擋「同一個元件兩種模式共用一個寬度」（`RelationPickerModal`
    的編輯只有一格下拉、新增有整份候選清單，兩者不該一樣寬）。
    驗法是**把寬度往下調一階看文字行數變不變**，不是憑感覺
58. **分隔線的條件只判了一側**（見 §6.8）——「下面有東西就畫」的寫法，在**上面那堆是空的**
    時候會留下一條沒有理由的線。條件一律是「**兩邊都各有東西**」。
