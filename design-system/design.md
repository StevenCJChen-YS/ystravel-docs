...
**判準是「使用者需不需要讀它」，不是它長在哪個位置。**

| 讀取必要性 | 色階 | 長度與 RWD | 典型 |
|---|---|---|---|
| **必讀**——不讀就做不了決定，或有法定告知義務 | `text-toned` | **不限長、不隱藏、不截斷** | Modal 內文、狀態說明、個資告知文字 |
| **輔助定位**——不讀也能用，讀了更快進入狀況 | `text-muted` | ≤20 全形字、`<sm` 隱藏、`≥sm` `line-clamp-1` | 頁面副標、區塊副標、meta（時間戳／IP／計數） |
| **可忽略**——掃過就好，不承載資訊 | `text-dimmed` | 不限 | 空值 `—`、附屬代碼、裝飾性圖示（chevron／拖曳把手／ⓘ） |

> **「副標比 Modal 內文更淡」乍看反直覺，照這條原則卻是對的**——副標只負責定位，
> Modal 內文是要你據以做決定的。**用位置排序會排錯，用「要不要讀」才排得對。**

**這條同時解掉「說明太長」那類爭論。** 一段文字讀起來很雜時，先問它屬哪一階：

- 屬**必讀**（例：LINE 綁定的個資告知「我們只取得用於推送的識別碼，不會讀取您的聊天內容…」）
  → **不可以刪短、不可以手機隱藏**。它擠是**版位問題不是字數問題**——
  必讀的內容被塞進副標的位置，解法是給它自己的版位，不是砍字。
- 屬**輔助定位** → 壓到 20 字內，機制說明搬去 tooltip／內文（同 §10.4）。
...
28. 說明類文字憑感覺選色階，而不是照「使用者需不需要讀它」判定（見 §2.5.1）；
    尤其**把必讀內容（個資告知、Modal 內文）壓成副標色階再嫌它太長**——那是版位問題不是字數問題
