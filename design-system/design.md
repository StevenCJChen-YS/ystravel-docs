...
| 項目 | 內容 |
|---|---|
| 適用範圍 | `ystravel-platform`（`apps/portal` 與各模組前端）；未來各系統前端沿用同一套 Base |
...
3. 需要調整視覺 → 走 §11 的實作位置（theme seam），**不准 `!important` 或深層選擇器硬蓋**
...
| 條目層 | 大小 |
|---|---|
| 條目標題（類別，如「人事異動通知」） | `text-[15px] font-semibold` |
| 條目內文（事實句） | `text-[13px]` |
| 條目時間戳 | `text-xs` |
...
1. `!important` 或深層選擇器硬蓋官方元件樣式（改走 §11 的 theme seam）
