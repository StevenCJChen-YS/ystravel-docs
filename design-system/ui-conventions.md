...
AuthPortal `src/shared/ui` 共用元件（改任何管理頁 UI 前先看有沒有現成的，別重造）：`ToolbarButton`／`EmptyState`／`FilterPanel`／`ListItemButton`／`TableSortButton`／`OrderManager`／`ListPanel`（master-detail 左欄殼）等，另有 `TableCard`/`FormModal`/`DataToolbar`/`TableLoading`/`AppInput`/`AppSelect`…共 24+。
...
- **看到可抽共用就當下抽**（2026-07-13 ALWAYS）：表格外觀 → `vite.config` component theme；重複 UI → `shared/ui/`；重複狀態邏輯 → `shared/composables/`（useConfirm/useEditModal…）。寫任何頁面前先問「這段會不會有第二頁也要用」。已落地例：table theme 深表頭一處改全站、`TableLoading.vue`、`useEditModal.ts`（關閉延後清 target，修「對話框收場先變矮」）。
