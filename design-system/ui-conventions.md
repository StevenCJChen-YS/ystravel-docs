...
## 21. Nuxt UI 加新色時，`ui.colors` 與 `theme.colors` 是兩件事（2026-08-08 實測，分類色票事故）

**症狀**：`<UBadge color="cyan" variant="soft">` 的**文字顏色是對的，底色卻完全透明**
——標籤看起來像一行有顏色的字，不像 chip。**typecheck、測試、build 全綠，沒有任何東西報錯。**

| 設定 | 管什麼 |
|---|---|
| `ui.colors` | 產出 `--ui-{色}` **變數** |
| **`theme.colors`** | **產出 solid／outline／soft／subtle 四組變體的 class** |

badge 的 theme 是 `options.theme.colors.map(色 => 'bg-{色}/10 text-{色}')`——**變體 class 是照
`theme.colors` 這張清單產的**。只加 `ui.colors` 的結果就是「變數在、變體不在」。

⚠️ **`theme.colors` 給了就是取代不是合併**：`resolveColors` 只會自動補回 `primary`。
只列新加的色，success／warning／error **全站的徽章與按鈕會一起失去樣式**——
所以既有的語意色一定要一起列出來。

### 為什麼驗證會漏掉

當時驗的是「`--ui-cyan` 有沒有值」與「`text-cyan-700` 產不產得出來」，**兩個都過**。
問題是那兩項證明的是變數與 utility class 存在，**沒有一項證明變體 class 被產出來**。

📌 **可複用**：加新色之後的驗收條件是**把元件真的畫出來看**，不是查設定值在不在。
這是〈設定值存在 ≠ 生效〉的變形——**設定確實生效了，只是生效的不是你以為的那件事**，
所以「我驗過設定了」這句話在這裡完全沒有保護力。

📌 **改的時候一定要留對照組**：修完要順手看一個**既有的語意色**徽章有沒有被連帶弄壞
（本次用「來源」徽章 primary soft 確認 `bg-primary/10 ring-primary/25` 沒變）。
`theme.colors` 是取代語意，沒有對照組就看不出自己剛剛拆了什麼。

規範本體＝[design.md](./design.md) §2.8 分類色票。


---

## 22. `truncate` 與 inline 元素：一個藏了八天的洞（2026-08-27）

**規範在 `design.md` §3.5，這裡記的是怎麼被抓到的。**

### 抓到的方式：Steven 看畫面，我去量

他的原話是「**我剛剛看一下好像名字的上方有一點空白，我不知道該怎麼說，反正我覺得
現在中間內容整體沒有高度置中，好像差一點點**」——沒有指出任何機制，就是覺得怪。

量下去是 **3px**：卡片頂→姓名文字頂 16px，副標文字底→卡片底 13px。
而且**根因不是 padding**（`li` 的 `py-3` 上下對稱、`items-center` 也對），
是姓名那個 `<a>` 是 inline，行框高度被外層 `div` 的支柱撐開（16px/24px vs 14px/20px）。

### 一個症狀帶出另一個

去查 inline 的行為時才發現**同一個元素上的 `truncate` 也是啞的**——
`overflow: hidden` 不作用在 inline 非置換元素上。實測 36 字的姓名溢出容器 91.7px。
**那個洞從 2026-08-19 親友清單上線就在**，八天沒人發現，因為中文姓名根本長不到那個程度
（那一格約放得下 26 個字）。

📌 **可複用的兩點：**

1. **驗一個 CSS 修法要把候選方案都量過再選**，不要挑第一個會動的。
   這次量了三種：`inline`（現況，差 3px、溢出 91.7px）、`block`（差 0，但熱區 42px→412px）、
   `block w-fit max-w-full`（差 0、熱區仍 42px、溢出 0）。**只有第三個三件事都對。**
2. **「看起來只差幾 px」的問題，成因常常不在 padding**。padding 對稱、`items-center`
   也設了，還是不置中，那就去看**行框**：字級、行高、以及外層那個看不見的支柱。

### 順帶記一條同族的

同一天同一張卡片外面那層框（`InsetFrame`）也被 Steven 指出「padding 是不是沒設計好」
——那個是真的內距問題：`InsetFrame` **刻意不給內距**（它只出外框），垂直呼吸一律由
使用點自己給，而我只給了 `px-5`。
📌 **抽共用容器時，「這一層負責什麼、不負責什麼」要寫在元件檔頭**，
不然下一個使用點就會漏掉它不負責的那一半。`InsetFrame` 的檔頭寫了，是我沒讀。
