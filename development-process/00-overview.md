...
>
> **2026-07-29 補**:尺寸表補上「**機制**」一格與 `/brainstorm`,並補上**判斷順序(三刀)**。原本四格漏掉
...
#### 判斷順序:三刀(2026-07-29 補)

上表是速查用的,**實際判的時候照這個順序切三刀**。

**這張表本來就不是單一軸**——微改/中等/大是尺寸、迷霧是清晰度、機制是性質。
它真正在回答的是「走什麼流程」,左欄只是標籤。所以**順序比表格本身重要**,
不要只問「這個大不大」。

**第零刀**:連「要做什麼」都講不清楚 -> **迷霧**,先撥霧,撥完再回來走下面兩刀。

**第一刀(主基準):這件事要不要「定規則」?** 問三題,**任一題答「是」就算有規則**:

1. 要不要往 `CONTEXT.md` 加新詞?
2. 寫得出「**誰**在**什麼情況**下**可以做什麼**」嗎?
3. 要不要新增**要存起來、要有人維護**的資料?

有規則 -> **要寫規格**(中等 / 大);沒規則 -> **不寫規格**(微改 / 機制)。

**第二刀:兩半的次要基準不一樣**,這點最容易搞混:

| 那一半 | 分界 | 落點 |
|---|---|---|
| **沒規則** | **做錯了,畫面看不看得出來** | 看得出來 -> **微改**;看不出來 -> **機制** |
| **有規則** | 一個 context 塞不塞得下 | 塞得下 -> **中等**;塞不下 -> **大**(切票) |

> **一句話版**:先問「要不要定規則」——要,就寫規格;不要,再問「做錯了畫面看不看得出來」
> ——看不出來就先 `/brainstorm`,看得出來就直接做。
...
**與微改的分界不是「大不大」,是「做錯了,畫面看不看得出來」。**
微改做完看畫面就知道對不對,錯了一眼看得出來、改回去很便宜;
機制做完**看起來「像是對的」,但方向可能整個錯,而畫面不會告訴你**。

> **兩個反例,很容易判錯,要記**:
>
> **① 該判機制卻判成微改**:全站介面縮放如果做成**等比例放大**,畫面看起來完全正常
> ——但那正是瀏覽器 Ctrl +/- 已經在做的事,也正是原本的問題本身(一頁塞不下東西);
> 要的是「字大、密度守住」。**這種錯畫面不會報給你**,所以值得先跑一輪對齊方向。
>
> **② 該判微改卻判成機制**:使用者管理按鈕收進 `⋯`(platform #106)**也抽了共用元件**
> `RowActionsMenu`,但它是微改——收錯了一眼就看得出來。
> **所以判準不是「有沒有碰共用層」**;碰共用層常常是相關訊號,但不是判準本身。
...
**走完長什麼樣**:`/brainstorm` -> 計劃書存**該專案** `docs/features/<slug>/decision.md`
...
> **計劃書放哪(2026-07-30 拍板結案,取代 07-29 的待決註記)**:**專案專屬的進該專案
> `docs/features/<slug>/decision.md`**(形狀 B),`<slug>` 跟 `feature/<slug>` 分支同名;
> 跨專案或還不確定是哪個專案的才留 my-agent `100_Todo/projects/`。
> **理由不是口訣,是已經在痛**——計劃書放 my-agent,在專案 repo 裡工作的人讀不到,
> 於是決策被迫寄生到別的文件上:介面縮放最值錢的那條判準(「這個偏好描述的是**環境**
> 還是**身體**」)現在活在 platform `roadmap.md` 的 W0 段落裡,而 roadmap 該管的只有時程與排序。
> 對照過既有計劃書,形狀 B 要的五項(狀態/拍板人/日期/背景/為什麼不選另一個做法)全中,
> **格式不用改,以前只是投錯 repo**。
>
> ⚠️ 兩個邊界:①計劃書裡的「**具體步驟 Step 1～N**」是一次性施工單,做完就過期,
> **不進 repo**(留 PR 描述或 scratchpad),`decision.md` 只收決策那半;
> ②**既有兩份(側邊欄搜尋、介面縮放)刻意不回頭搬**——功能都上線了、決策也留下來了,
> 搬過去零新增資訊。**規則從下一個機制尺寸的題目開始生效**,而目前佇列裡沒有,
> 所以這條規則還沒被真實案例驗過(照 07-29 那條:寫完規則要拿真實案例判一遍)。
...
- **機制型改動**(改「東西怎麼運作」而非「業務規則是什麼」,例如全站字級刻度、頁面搜尋):
  不寫 PRD/SA/SD,但**要先跑 `/brainstorm` 產計劃書**。**與上一條純 UI 調整的分界,是
  「做錯了畫面看不看得出來」**——純 UI 調整錯了一眼看得出來,機制型做完會「看起來像是對的」
  而方向整個錯(介面縮放做成等比例放大就是例子:畫面正常,但那正是原本的問題)。
  三條判準與反例見 §3。⚠️ **一旦要新增資料表或維護介面,就掉回下一條「新功能」,從 PRD 開始。**
