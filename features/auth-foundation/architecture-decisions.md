...
- **Token 簽章**：第一階段先恢復可用；後續必須把 access token 從 HS256 共用密鑰升級為 RS256/JWKS（非對稱金鑰），避免多系統共用 secret 的風險。
...
| D4 | SSO 協定 | **自有 JWT + 後續 RS256/JWKS** | 先恢復可用，再把多系統信任模型升級到非對稱驗簽 |
...
| D7 | Token 驗證 | **第一階段沿用現有 JWT；後續升級 RS256/JWKS** | 先讓 native flow 回來，再修掉 HS256 共用密鑰風險 |
...
- **後續必修**：access token 升級為 RS256/JWKS，讓各系統只持有公鑰，不共享簽章 secret。
...
| 欄位 | 內容 |
|---|---|
| 認證與授權方式 | AuthService native login + JWT；後續升級 RS256/JWKS；權限集中定義於 AuthService、各系統 `PermissionsGuard` 執行 |
...
- **secrets**：JWT secret / 後續 RSA private key、DB 連線字串全走環境變數，不進 repo。
...
2. **Phase 1b**：修 token 安全：HS256 → RS256/JWKS，補 key rotation 與各系統驗簽規則。
