...
| POST/PATCH | /admin/org/departments(/:id) | 建/改/停用/排序部門（停用前檢查無成員） | AUTH.ORG.MANAGE |
...
| Audit log | ORG_COMPANY_/ORG_DEPARTMENT_ CREATED/UPDATED/DISABLED；GROUP_CREATED/UPDATED/MEMBER_ADDED/MEMBER_REMOVED；ROLE_ASSIGNED/ROLE_REMOVED（含 expiresAt）；**SUPER_ADMIN_ASSIGNED/REMOVED（必記，含操作者與對象）**；USER_DEACTIVATED（含操作者，HR 停用可追溯） |
