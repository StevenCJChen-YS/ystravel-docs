...
| DELETE | /admin/groups/:id | 硬刪除群組（成員關聯 cascade；停用＝可逆日常、刪除＝確定不用；audit 前存 code/name/成員數） | AUTH.GROUP.MANAGE |
| GET | /admin/groups/:id/members | 成員名單（含停用標示） | AUTH.GROUP.MANAGE |
| POST/DELETE | /admin/groups/:id/members(/:userId) | 加/移除單一成員 | AUTH.GROUP.MANAGE |
| POST | /admin/groups/:id/members/batch | 批次加入（body `userIds[]`，一次交易 createMany＋逐人 audit；忽略無效/已在名單的，回傳實際新增數） | AUTH.GROUP.MANAGE |
...
**稽核查詢（`GET /audit-logs`）**：`q` 關鍵字除了操作者/對象的帳號、姓名、Email，另加搜 `diff` JSON 內容（`string_contains`）——群組名/公司名等「摘要」關鍵字都存在 diff 裡才搜得到。限制：摘要開頭的中文動作名是前端翻譯、DB 無此字串，搜不到；要按動作找走「動作」下拉。
