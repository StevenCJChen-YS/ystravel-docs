...
**正向變體最會騙人**：migration **新增**欄位後（如 `groups.isSystem`），舊 client 不知道有這欄 → seed 噴的是 **validation error `Unknown argument 'isSystem'`**，而它會把整個 `create` 物件印出來、用 `~~~~` 標在那個欄位下面，還列出「Available options」——**看起來就像 seed 寫錯欄位名或 schema 沒加**，很容易跑去讀 `seed.ts` 和 `schema.prisma` 找不存在的錯字。**判準：`Unknown argument` 指著的欄位如果確實存在於 `schema.prisma`，那就是 client 過期，不是寫錯。** 注意這一條連 migration 都套用成功了（DB 是對的），只有 client 落後。（2026-07-28 家用機 pull 後 seed 中招）
